---
title: '@masonry-grid/solid-js'
description: Documentation for the SolidJS package of Masonry Grid
sidebar:
  order: 4
---
...
- `frameWidth?: number | string` - Minimum width of each frame. Used to calculate `grid-template-columns`. Can be a number (px) or string with units, or a reactive accessor.
- `gap?: number | string` - Gap between items. Can be a number (px) or string with units, or a reactive accessor.
...
```tsx
import { Frame } from '@masonry-grid/solid-js'
